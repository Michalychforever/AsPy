...
#sample k-space lineary or in the log-mode (log-mode is better for the dipole)
...
#use standard integration routines or the customary ones, where you can control the accuracy 
...
#deltaLbar = interpolate.InterpolatedUnivariateSpline(rinvals,deltaLbartab,bbox=[rinvals[0], rinvals[-1]],ext=3)
    ...
    for kindex1 in range(Nk+1):
        ...
        for k in range(1,Nt):
            MattB[k][k-1] = 1./tau/tau - 1./tau/4.
            MattB[k][k] = -2./tau/tau -0.5 + P1[k]
            MattB[k][k+1] = 1./tau/tau + 1./tau/4.
            MattB[k][(Nt+1)+k] = 1.5/rBG[k][jRin]**2.
        for k in range(Nt):
    ...
    for k in range(1,Nt):
        MattA[k][k-1] = 1./tau/tau - 1./tau/4.
        MattA[k][k] = -2./tau/tau -0.5 + P1[k]
        MattA[k][k+1] = 1./tau/tau + 1./tau/4.
        MattA[k][(Nt+1)+k] = 1.5/rBG[k][jRin]**2.
    for k in range(Nt):
