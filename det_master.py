...
Nx = 1000
Nt = 500
jRin = 150  #number of Rin in the array of coordinates

rmin = 1.e-2 #this value is used in the finite difference method
r0 = 1.
rmin2 = 1.e-5 #this value is used in the calcualion of the saddle-point profile
...
# sampling k-space logarithmically or linearly
log_mode = False
#log_mode = True
...
# decide if you want to compute the whole determinant, ot just exp(tr)
det_mode = True

# use the standard scipy functions for integration or the manual ones (slower, but more accuracate)
...
detQP = np.zeros(len(dWtab))
traceQP = np.zeros(len(dWtab))
...
for dWindex in range(len(dWtab)):
    ...
    if standard_integration>0:
        ...
        rinvals = np.linspace(log(rmin2),log(rmax),1000)
        rinvals = exp(rinvals)
        ...
        rinvals2 = np.linspace(rinvals[0],rinvals[-1],1000)
        ...
        rinvals = np.linspace(rmin2,rmax,1000)
...
#        deltaLbar = interpolate.InterpolatedUnivariateSpline(rinvals,deltaLbartab,bbox=[rinvals[0], rinvals[-1]],ext=3)
        ...
        for j in range(len(rinvals)):
            ...
            for i in range(Nint4):
                ...
                integrand4[i] = Plin(integrandk4[i])*W(integrandk4[i]*Rin)*cos(integrandk4[i]*rinvals[j])*integrandk4[i]**2./(2.*pi**2.)/rinvals[j]
        ...
        deltaLprime1 = interpolate.InterpolatedUnivariateSpline(rinvals,deltaLprimetab1,bbox=[rinvals[0], rinvals[-1]],ext=3)
        deltaLprime = lambda x: (-1.*xiRin(x)/x + deltaLprime1(x))*F(1.+deltaW)/SigmaSquareRofdeltaW
    ...
    A = -0.5 + 2.*(TThetaBGmat - 2.*drPsiBGmat/rBG)
    ...
    for kindex in range(Nk+1):
        ...
        for j in range(Nx+1):
            ...
            psi[kindex][0][j] = -u[kindex][0][j]/ktab[kindex]**2.
        ...
        for k in range(Nt):
            ...
            b = np.zeros(3*(Nx + 1))
            for j in range(Nx):
                b[j+Nx+1] = u[kindex][k][j]*(1./tau + A[k][j]/2.  - l*(-1.*drPsiBGmat[k][j])/rBG[k][j]*(1. - pow(rBG[k][j],l)/( pow(r0,l) + pow(rBG[k][j],l) ))/2.) + 1.5/2.*rho[kindex][k][j]+C[k][j]/h/2.*psi[kindex][k][j+1] - psi[kindex][k][j]/2.*(C[k][j]* (1./h-1.*l*pow(r0,l)*drdrinBG[k][j]/(pow(rBG[k][j],l)+pow(r0,l))/rBG[k][j])-D[k][j]*l*(l+1))
            ...
            b[Nx] = -1.*b[2*(Nx+1)-1]/ktab[kindex]**2.
    ...
    np.savetxt('Wavefunctions_at_Rin_for_l'+str(l)+'_dW'+str(deltaW)+'_x'+str(Nx)+'_t'+str(Nt)+'_Nk'+str(Nk)+'_.dat', WFofktab)
    ...
    L1 = np.zeros(Nt+1)
    for k in range(Nt+1):
        P1[k] = 1.5*G[k][jRin]-(Rin/rBG[k][jRin])**3.
    ...
    if det_mode >0:
        ...
        for kindex1 in range(Nk+1):
            for kindex2 in range(kindex1+1):
                for k in range(Nt+1):
                ...
                UpsDeltaini = exp(2.*etamin)/(4.*np.pi)* (bessel1*(-1./ktab[kindex2]**2.)*derivbessel2 + bessel2*(-1./ktab[kindex1]**2.)*derivbessel1)/2.
                UspThetaini = exp(2.*etamin)/(4.*np.pi)*(bessel1*(-1./ktab[kindex2]**2.)*derivbessel2 + bessel2*(-1./ktab[kindex1]**2.)*derivbessel1 - 2.*2./Rin/ktab[kindex1]**2./ktab[kindex2]**2.*derivbessel1*derivbessel2 + 2.*l*(l+1)/Rin**2./ktab[kindex1]**2./ktab[kindex2]**2.*(bessel1*derivbessel2 + bessel2*derivbessel1)-2.*l*(l+1)/ktab[kindex1]**2./ktab[kindex2]**2./Rin**3.*bessel1*bessel2)/2.
                ...
                for k in range(1,Nt):
                    Matt[k][k-1] = 1./tau/tau - 1./tau/4.
                    Matt[k][k] = -2./tau/tau -0.5 + P1[k]
                    Matt[k][k+1] = 1./tau/tau + 1./tau/4.
                    Matt[k][(Nt+1)+k] = 1.5/rBG[k][jRin]**2.
                for k in range(Nt):
	...
	### using symmetry of the matrix 
        ...
        np.savetxt('Determinant_mode_Q_and_Ups_l'+str(l)+'_dW'+str(deltaW)+'_Nk'+str(Nk)+'_.dat', Qofkstab)
        ...
        QPmatrix = np.zeros((Nk+1,Nk+1))
        
        if log_mode>0:
            dlnk = (log(ktab[Nk])-log(ktab[0]))/Nk
            for kindex1 in range(Nk+1):
                for kindex2 in range(Nk+1):
                    QPmatrix[kindex1][kindex2] = 2.*lambdaStar*dlnk*sqrt(Plin(ktab[kindex1]))*sqrt(Plin(ktab[kindex2]))*mustartab[kindex1][kindex2]*3./R**3./8./np.pi**3.*exp(1.5*log(ktab[kindex1]))*exp(1.5*log(ktab[kindex2]))
            
        else:
            dk = (kmax - kmin)/Nk
            for kindex1 in range(Nk+1):
                for kindex2 in range(Nk+1):
                    QPmatrix[kindex1][kindex2] = 2.*lambdaStar*dk*sqrt(Plin(ktab[kindex1]))*sqrt(Plin(ktab[kindex2]))*mustartab[kindex1][kindex2]*3./R**3./8./np.pi**3.*ktab[kindex1]*ktab[kindex2]


        traceQP[dWindex] = np.trace(QPmatrix)
        
        ### adding the Kronecker delta
        
        for kindex1 in range(Nk+1):
            QPmatrix[kindex1][kindex1] = QPmatrix[kindex1][kindex1] + 1.


        detQP[dWindex] = np.linalg.det(QPmatrix)

        Preftab = [ [0 for x in range(3)] for y in range(1)];
        Preftab[0][0] = dWtab[dWindex]
        Preftab[0][1] = exp(traceQP[dWindex])
        Preftab[0][2] = detQP[dWindex]
        np.savetxt('Determinant_l'+str(l)+'_Nk'+str(Nk)+'_dW'+str(deltaW)+'_.dat', Preftab)

    else:

        mustartab = np.zeros((Nk+1))
        UpsilonDelta0 = np.zeros((Nk+1))
        UpsilonTheta0 = np.zeros((Nk+1))
        UpsilonDeltaRin = np.zeros((Nk+1,Nt+1))
        UpsilonThetaRin = np.zeros((Nk+1,Nt+1))

        for kindex1 in range(Nk+1):
            for k in range(Nt+1):
                UpsilonDeltaRin[kindex1][k] = 1./(4.*np.pi)*(DeltaRin[kindex1][k]*PsiPrimeRin[kindex1][k])
                UpsilonThetaRin[kindex1][k] = 1./(4.*np.pi)*(ThetaRin[kindex1][k]*PsiPrimeRin[kindex1][k] - 2./rBG[k][jRin]*(PsiPrimeRin[kindex1][k]*PsiPrimeRin[kindex1][k])+ 2.*l*(l+1)*PsiPrimeRin[kindex1][k]*PsiRin[kindex1][k]/rBG[k][jRin]**2.-1.*l*(l + 1)*PsiRin[kindex1][k]*PsiRin[kindex1][k]/rBG[k][jRin]**3.)

            UpsilonDelta0[kindex1] = UpsilonDeltaRin[kindex1][Nt]
            UpsilonTheta0[kindex1] = UpsilonThetaRin[kindex1][Nt]


                ### solving the ODE's for mu2 and r2

            bessel1 = 4.*np.pi*scipy.special.spherical_jn(l,ktab[kindex1]*Rin)
            bessel2 = 4.*np.pi*scipy.special.spherical_jn(l,ktab[kindex1]*Rin)
            derivbessel1 = (1.*l/Rin*scipy.special.spherical_jn(l,ktab[kindex1]*Rin)-scipy.special.spherical_jn(l+1,ktab[kindex1]*Rin)*ktab[kindex1]) *(4.*np.pi)
            derivbessel2 = (1.*l/Rin*scipy.special.spherical_jn(l,ktab[kindex1]*Rin)-scipy.special.spherical_jn(l+1,ktab[kindex1]*Rin)*ktab[kindex1]) *(4.*np.pi)

            UpsDeltaini = exp(2.*etamin)/(4.*np.pi)* (bessel1*(-1./ktab[kindex1]**2.)*derivbessel2 + bessel2*(-1./ktab[kindex1]**2.)*derivbessel1)/2.
            UspThetaini = exp(2.*etamin)/(4.*np.pi)*(bessel1*(-1./ktab[kindex1]**2.)*derivbessel2 + bessel2*(-1./ktab[kindex1]**2.)*derivbessel1 - 2.*2./Rin/ktab[kindex1]**2./ktab[kindex1]**2.*derivbessel1*derivbessel2 + 2.*l*(l+1)/Rin**2./ktab[kindex1]**2./ktab[kindex1]**2.*(bessel1*derivbessel2 + bessel2*derivbessel1)-2.*l*(l+1)/ktab[kindex1]**2./ktab[kindex1]**2./Rin**3.*bessel1*bessel2)/2.

            bt = np.zeros(2*(Nt + 1))
            for k in range(1,Nt):
                bt[k+Nt+1] = (rBG[k][jRin]**2.*UpsilonDeltaRin[kindex1][k] + rBG[k+1][jRin]**2.*UpsilonDeltaRin[kindex1][k+1])/2.
                bt[k] = -1.*UpsilonThetaRin[kindex1][k]

            bt[Nt+1] = (rBG[0][jRin]**2.*UpsilonDeltaRin[kindex1][0]+rBG[1][jRin]**2.*UpsilonDeltaRin[kindex1][1])/2.
            bt[2*(Nt+1)-1] = Rin**2.*(5.*UpsDeltaini+2.*UspThetaini)/7.
            bt[0] = -2.*Rin**2.*(3.*UpsDeltaini + 4.*UspThetaini)/14.
            ...
            mustartab[kindex1] = mu2[Nt]

        t3 = time()
        print('Mu2 and r2 equations are solved in (sec.),', t3-t2)


        Qofkstab = [ [0 for x in range(4)] for y in range((Nk+1))];
        for kindex1 in range(Nk+1):
            Qofkstab[kindex1][0] = ktab[kindex1]
            Qofkstab[kindex1][1] = UpsilonDelta0[kindex1]
            Qofkstab[kindex1][2] = UpsilonTheta0[kindex1]
            Qofkstab[kindex1][3] = mustartab[kindex1]*3./R**3.
        np.savetxt('Trace_mode_Q_and_Ups_of_k_l'+str(l)+'_dW'+str(deltaW)+'_Nk'+str(Nk)+'_.dat', Qofkstab)
        ...
        if log_mode>0:
            dlnk = (log(ktab[Nk])-log(ktab[0]))/Nk
            for kindex1 in range(Nk+1):
                QPmatrix[kindex1][kindex1] = 2.*lambdaStar*dlnk*Plin(ktab[kindex1])*mustartab[kindex1]*3./R**3./8./np.pi**3.*exp(3*log(ktab[kindex1]))

        else:
            dk = (kmax - kmin)/Nk
            for kindex1 in range(Nk+1):
                QPmatrix[kindex1][kindex1] = 2.*lambdaStar*dk*Plin(ktab[kindex1])*mustartab[kindex1]*3./R**3./8./np.pi**3.*ktab[kindex1]**2.

        traceQP[dWindex] = np.trace(QPmatrix)
        detQP[dWindex] = exp(traceQP[dWindex])

        Preftab = [ [0 for x in range(2)] for y in range(1)];
        Preftab[0][0] = dWtab[dWindex]
        Preftab[0][1] = detQP[dWindex]
        np.savetxt('Trace_l'+str(l)+'_Nk'+str(Nk)+'_dW'+str(deltaW)+'_.dat', Preftab)

#Preftab = [ [0 for x in range(3)] for y in range(len(dWtab))];
#for dWindex in range(len(dWtab)):
#    Preftab[dWindex][0] = dWtab[dWindex]
#    Preftab[dWindex][1] = exp(traceQP[dWindex])
#    Preftab[dWindex][2] = detQP[dWindex]
#np.savetxt('Determinant_l'+str(l)+'_Nk'+str(Nk)+'_.dat', Preftab)
